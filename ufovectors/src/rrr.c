...
zero_nint_index_t convert_one_nint_index_to_zero_nint_index(one_nint_index_t index) {    
    if (is_one_nint_index_na(index)) {
        ...
        make_sure(index.noix == 0, Rf_error, 
...
zero_cint_index_t convert_one_cint_index_to_zero_cint_index(one_cint_index_t index) {    
    make_sure(index.oix == 0 || index.oix == NA_INTEGER, Rf_error, 
...
integer_vector_t integer_vector_from(SEXP/*INTSXP*/ sexp) {
    make_sure(TYPEOF(sexp) == INTSXP, Rf_error, 
...
//     make_sure(!zero_based_int_index_is_na(index), Rf_error,
...
//     make_sure(!one_based_int_index_is_na(index), Rf_error,
//               "Cannot cast index with value NA to zero_based_not_na_int_index_t");
...
//     make_sure(index.zix == NA_INTEGER, Rf_warning, 
...
//     make_sure(index.oix == 0, Rf_error, "One-based integer index has value 0.");
