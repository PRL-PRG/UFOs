---
title: "Microbenchmarks"
author: "K. Siek"
date: "May 28 2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)


plot_to_pdf_and_here <- function(path, data) {
  pdf(path, width = 4, height = 4)
  plot(data)
  dev.off()
  
  plot(data)
}
```

```R
devtools::install_github("olafmersmann/microbenchmarkCore")
devtools::install_github("olafmersmann/microbenchmark")
```

```{r}
library(ufovectors)
library(ufoaltrep)
library(microbenchmark)
library(ggplot2)
library(scales)
```

```R
setwd("~/Workspace/ufo_workspace/UFOs/ufovectors/benchmark")
ufo_set_debug_mode(F)
options(scipen=999) # effectively turns off scientific notation
```

## Generating files

```{r generate}
generate_binary_file <- function(path, range, repeats) {
  sum <- 0
  minimum <- integer(0)
  maximum <- integer(0)
  size <- length(range) * repeats
  f <- file(path, "wb")
  for (i in 0:repeats) {
    writeBin(range, f)
    sum <- sum + sum(range, na.rm = TRUE)
    minimum <- min(minimum, range, na.rm = TRUE)
    maximum <- max(maximum, range, na.rm = TRUE)
  }
  close(f)
  list(path=path, sum=sum, size=size, min=minimum, max=maximum)
}

stats <- list(
  stats_1s = generate_binary_file("250M_1s.bin", as.integer(rep(1, 1000000)), 250), #126MB
  stats_seq_ints = generate_binary_file("250M_seq_ints.bin", 1:1000000, 250),
  stats_rand_ints = generate_binary_file("250M_rand_ints.bin", sample(1000000, replace=T), 250)
)

size <- 250 * 1000 * 1000
some_function <- function(x) x

print(stats)

file.exists(stats$stats_1s$path)

```

## File-backed matrices

Two nigh-identical implementations of file-backed vectors. One done with UFOs, one done with vectors.


### Creation: 32 million 1s

```{r creat, eval = FALSE, echo = FALSE}
result <- microbenchmark(
  "UFO" = { 
    ufo_integer_bin(stats$stats_1s$path, read_only=FALSE)
  },
  "UFO/RO" = { 
    ufo_integer_bin(stats$stats_1s$path, read_only=TRUE)
  },
  "ALTREP" = {
    altrep_ufo_integer_bin(stats$stats_1s$path)
  },
  times = 1000L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("File-backed/250M 1s/create")

plot_to_pdf_and_here("250M_1s_bin_create.pdf", plot)
```

### Sum: 32 million 1s

```{r fb-sum32mln1, cache=T, fig.height=3, fig.width=3, eval = FALSE, echo = FALSE}
ufo <- ufo_integer_bin(stats$stats_1s$path, read_only=FALSE)
ufo.ro <- ufo_integer_bin(stats$stats_1s$path, read_only=TRUE)
altrep <- altrep_ufo_integer_bin(stats$stats_1s$path)

result <- microbenchmark(
  "UFO" = { sum(ufo) },
  "UFO/RO" = { sum(ufo.ro) },
  "ALTREP" = { sum(altrep) },
  check = function(values) {
    all(sapply(values, function(result) result == stats$stats_1s$sum))
  },
  times = 1000L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("File-backed/250M 1s/sum")

plot_to_pdf_and_here("250M_1s_bin_sum.pdf", plot)
```
### Loop: 32 million 1s

```{r fb-for32mln1s, cache=T, eval = FALSE, echo = FALSE}
some_function <- function(x) x

ufo <- ufo_integer_bin(stats$stats_1s$path, read_only=FALSE)
ufo.ro <- ufo_integer_bin(stats$stats_1s$path, read_only=TRUE)
altrep <- altrep_ufo_integer_bin(stats$stats_1s$path)

result <- microbenchmark(
  "UFO" = {
    for (e in ufo) some_function(e)
  },
  "UFO/RO" = {
    for (e in ufo) some_function(e)
  },
  "ALTREP" = {
    for (e in altrep) some_function(e)
  },
  
  times = 10L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("File-backed/250M 1s/loop")

plot_to_pdf_and_here("250M_1s_bin_loop.pdf", plot)
```

### Creation: 32 million random integers

```{r creat32}
result <- microbenchmark(
  "UFO" = { 
    ufo_integer_bin(stats$stats_rand_ints$path, read_only=FALSE)
  },
  "UFO/RO" = { 
    ufo_integer_bin(stats$stats_rand_ints$path, read_only=TRUE)
  },
  "ALTREP" = {
    altrep_ufo_integer_bin(stats$stats_rand_ints$path)
  },
  times = 1000L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("File-backed/250M rand ints/create")

plot_to_pdf_and_here("250M_rand_bin_create.pdf", plot)
```

### Sum: 32 million random integers

```{r fb-sum32mlnrand, cache=T,}
ufo <- ufo_integer_bin(stats$stats_rand_ints$path, read_only=FALSE)
ufo.ro <- ufo_integer_bin(stats$stats_rand_ints$path, read_only=TRUE)
altrep <- altrep_ufo_integer_bin(stats$stats_rand_ints$path)

result <- microbenchmark(
  "UFO" = { sum(ufo) },
  "UFO/RO" = { sum(ufo.ro) },
  "ALTREP" = { sum(altrep) },
  check = function(values) {
    all(sapply(values, function(result) result == stats$stats_rand_ints$sum))
  },
  times = 1000L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("File-backed/250M rand ints/sum")

plot_to_pdf_and_here("250M_rand_bin_sum.pdf", plot)
```

### For loop: 32 million random integers

```{r fb-for32mlnrand, cache=T}
some_function <- function(x) x

ufo <- ufo_integer_bin(stats$stats_rand_ints$path, read_only=FALSE)
ufo.ro <- ufo_integer_bin(stats$stats_rand_ints$path, read_only=TRUE)
altrep <- altrep_ufo_integer_bin(stats$stats_rand_ints$path)

result <- microbenchmark(
  "UFO" = {
    for (e in ufo) some_function(e)
  },
  "UFO/RO" = {
    for (e in ufo) some_function(e)
  },
  "ALTREP" = {
    for (e in altrep) some_function(e)
  },
  
  times = 10L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("File-backed/250M rand ints/loop")

plot_to_pdf_and_here("250M_rand_bin_loop.pdf", plot)
```

ALTREP is really slow because:
- it has to do dispatch on each element
- it has to read from disk on each element

TODO: add an ALTREP implementation that buffers elements.

## Sequences

Here I'm comparing 

- our implementation of sequences, vs.
- `seq.int` which just returns an ordinary R vector, vs. 
- a compact vector (result of `n:m`), which, *I believe*, is implemented with ALTREP

## Creation

```{r seq-create, cache=T}
result <- microbenchmark(
  "UFO" = { ufo_integer_seq(1, size, 1, read_only = FALSE) },
  "UFO/RO" = {  ufo_integer_seq(1, size, 1, read_only = TRUE) },
  "ALTREP" = {  altrep_integer_seq(1, size, 1) },
  "R vector" = { as.integer(c(1:size)) },
  #"compact vector" = { 1:size },
  #"seq.int" = { seq.int(1, size, 1) },
  times = 100L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("Sequence/250M ints/create")

plot_to_pdf_and_here("250M_seq_create.pdf", plot)
```

## Sum: 32 mln int sequence

```{r seq-sum32mln, cache=T}
ufo <- ufo_integer_seq(1, size, 1, read_only = FALSE)
ufo.ro <- ufo_integer_seq(1, size, 1, read_only = TRUE)
altrep <- altrep_integer_seq(1, size, 1)
vec.std <- as.integer(c(1:size))
vec.comp <- 1:size
vec.seq.int <-  seq.int(1, size, 1)

result <- microbenchmark(
  "UFO" = { sum(ufo) },
  "UFO/RO" = { sum(ufo.ro) },
  "ALTREP" = { sum(altrep) },
  "R vector" = { sum(vec.std) },
  #"compact vector" = { sum(vec.comp) },
  #"seq.int" = { sum(vec.seq.int) },
  check = function(values) {
    all(sapply(values, function(result) result == sum(1:size)))
  },
  times = 1000L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("Sequence/250M ints/sum")

plot_to_pdf_and_here("250M_seq_sum.pdf", plot)
```

The compact vectors implement their own sum, which doesn't go through all the elements, instead it just calculates the sum from the boundaries of the sequence. I removed them in the end.

```C
(size / 2.0) * (n1 + n1 + inc * (size - 1))
```

## For loop: 32 mln int sequence

```{r fseq-for32mln, cache=T}
ufo <- ufo_integer_seq(1, size, 1, read_only = FALSE)
ufo.ro <- ufo_integer_seq(1, size, 1, read_only = TRUE)
altrep <- altrep_integer_seq(1, size, 1)
vec.std <- as.integer(c(1:size))
vec.comp <- 1:size
vec.seq.int <-  seq.int(1, size, 1)

result <- microbenchmark(
  "UFO" = {
    for (e in ufo) some_function(e)
  },
  "UFO/RO" = {
    for (e in ufo.ro) some_function(e)
  },
  "ALTREP" = {
    for (e in altrep) some_function(e)
  },
  #"seq.int" = {
  #  for (e in vec.seq.int) some_function(e)
  #},
  #"compact vector" = {
  #  for (e in vec.comp) some_function(e)
  #},
  "R vector" = {
    for (e in vec.std) some_function(e)
  },
  times = 10L
)

plot <- autoplot(result) +
  scale_y_continuous(labels = scales::label_number_si()) +
  xlab("time [ns]") +
  ggtitle("Sequence/250M ints/loop")

plot_to_pdf_and_here("250M_seq_loop.pdf", plot)
```