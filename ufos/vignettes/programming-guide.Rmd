...
## The C function that creates a vector, part 1
...
SEXP/*INTXP*/ ufo_seq(SEXP/*INTXP*/ from, 
                      SEXP/*INTXP*/ to, 
                      SEXP/*INTXP*/ by);
```
...
you need to work with UFOs:
  ...
  - `ufo_vector_type_t` - UFO vector type definitions, which are analogous to R vector types, and `ufo_type_to_vector_type`, a function to convert from one to the other,
  - `ufo_source_t` - a structure that passes the necessary configuration data to the UFO framework,
  - `ufo_initialize` and `ufo_shutdown` - functions for starting and shutting down the UFO framework,
...
type `ufo_source_t` that you passed in to `ufo_new`. This means it contains all
the necessary data for your vector to generate data, including the data you
passed as `source->data`.
...
int populate(uint64_t startValueIdx, uint64_t endValueIdx,
             ufPopulateCallout callout, 
             ufUserData src, char* target) {

    ufo_source_t* source = (ufo_source_t*) source;
    ufo_seq_data_t* data = (ufo_seq_data_t*) source->data;

    for (size_t i = 0; i < endValueIdx - startValueIdx; i++) {
        target[i] = data->from + (data->by - 1) * (i + startValueIdx);
    }
