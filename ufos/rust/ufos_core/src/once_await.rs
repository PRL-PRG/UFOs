use std::{sync::{Arc, Condvar, Mutex, atomic::{AtomicPtr, Ordering}}};
...
impl<T> OnceFulfiller<T> for OnceAwait<T> {
    fn try_init(&self, value: T){
        ...
        let res = self.value.compare_exchange(
            std::ptr::null_mut(), as_ptr, Ordering::Release, Ordering::Relaxed);
        match res {
            Ok(_) => {
...
impl<T> OnceAwait<T> {
    ...
    pub fn get(&self) -> &T {
        ...
        let _guard = self.condition
            .wait_while(guard, |_| std::ptr::null_mut() == self.value.load(Ordering::Acquire))
            .unwrap();
